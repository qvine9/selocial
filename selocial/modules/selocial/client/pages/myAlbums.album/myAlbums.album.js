/**
 * Edit album controller
 */
angular.module('selocial').controller('MyAlbums_albumPageController', function(api, uploader, error, notify, $rootScope, $scope, $reactive, $state, $stateParams, dialog, $timeout){
    ...
    page.subscribe('my-albums');
    page.subscribe('my-non-album-tracks')

    page.helpers({
        album: function () {
            return Album.findOne({_id: $stateParams.albumId});
        },
        tracks: function () {
            var ids = _.pluck(page.album.tracks, 'originalTrack');
            return Track.find({userId: Meteor.userId(), _id: {$nin: ids}}, {sort: {date: -1}}).fetch();
        }
    });
...
page.uploadCoverArt = function(append){
    if (page.album.images.length && page.album.images.length > 9) {
        dialog.confirm('Max 10 images allowed', {});
        return;
    }
    ...
    page.updatePageData = function(){
        ...
        page.album.tracks.forEach(t => page.album.duration += t.file.metadata.duration || 0);
    };
    ...
    page.addTrack = function(audioTrack){
        ...
        audioTrack.albumId = page.album._id;
    ...
    page.saveAlbum = function(){
        ...
        var album = {};
        Object.assign(album, page.album);
        var albumId = album._id;
        delete album._id;
        console.log(album);
        api('band.editAlbum', { album: album, albumId: albumId}).then(function(){
            notify.success('You have successfully updated your album!');
            $state.go('myAlbums');
        }).catch(error);
    };

    page.deleteAlbum = function() {
        console.log(page.album._id);
        dialog.confirm("WARNING! If this album is deleted then your fans will not be able to add your album to their playlists as well as YOUR ALBUM WILL GO MISSING within any fan mixes that contain your album!").then(function(confirmed){
            if(confirmed){
                api('band.deleteAlbum', { albumId: page.album._id }).then(function(){
                    notify.success('Your album has been successfully deleted!');
                    $state.go('myAlbums');
                }).catch(error);
            }
        });
